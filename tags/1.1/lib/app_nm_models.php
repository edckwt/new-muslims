...
class app_nm_models {
	...
	{
		global $opicnm_categories_lang;
		$return = array();
		$table = $this->wpdb->prefix.'options';
		$like = OPICNM_Input_SLUG."category_".get_option(OPICNM_Input_SLUG.'language').'_';
		...
		if(!empty($data) && is_array($data)){
			foreach ($data as $key => $value) {
				...
				$return[$key]['option_url'] = $opicnm_categories_lang[get_option(OPICNM_Input_SLUG.'language')][$return[$key]['option_slug']]['importurl'];
