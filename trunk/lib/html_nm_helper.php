 ...
 *
 */
class html_nm_helper extends app_nm_helpers {
	...
	public function Input($type = 'text', $attr = array()) {
		...
		switch ($type) {
			...
			default :
				{
					if(empty($attr['value'])){
						$attr['value'] = set_value(OPICNM_Input_SLUG.$attr['name']);
					}
					 
					 
					$html .= sprintf('<input type="%s" name="%s" value="%s" id="%s" />', $type, OPICNM_Input_SLUG.$attr['name'], $attr['value'], $attr['id']);
	...
	protected function _select($attr = array()) {
		...
		$option = get_option(OPICNM_Input_SLUG.$get_attr_name);
		
		$html = sprintf('<select name="%s" id="%s" >', OPICNM_Input_SLUG.$get_attr_name, $get_attr_id);
	...
	protected function _radio($attr = array()) {
		...
		foreach ($attr['options'] as $key => $value) {
			...
			$html .= sprintf('<p><input type="radio" name="%s" id="%s" value="%s"  %s /> <label for="%s">%s</label></p>', OPICNM_Input_SLUG.$attr['name'], $input_id, $key, $checked, $input_id, $value);
		}
	...
	protected function _checkbox($attr = array()) {
		...
		$option = get_option(OPICNM_Input_SLUG.str_replace('[]','',$get_attr_name));
		$html = '';
		$loop = 0;
		foreach ($get_attr_options as $key => $value) {
		    $loop++;
			...
			$html .= sprintf('<p><input type="checkbox" name="%s" id="%s" value="%s" %s /> <label for="%s">%s</label></p>', OPICNM_Input_SLUG.$get_attr_name, $input_id, $key, $checked, $input_id, $value);
		}
