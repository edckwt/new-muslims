...
class html_nm_helper extends app_nm_helpers {
	...
	protected function _radio($attr = array()) {
		foreach ($attr['options'] as $key => $value) {
			$checked = NULL;
			$html .= sprintf('<p><input type="radio" name="%s" id="%s" value="%s"  %s /> <span>%s</span></p>', OPICNM_Input_SLUG.$attr['name'], $attr['id'], $key, $checked, $value);
		}
	...
	protected function _checkbox($attr = array()) {
		...
		foreach ($get_attr_options as $key => $value) {
			...
			$html .= sprintf('<p><input type="checkbox" name="%s" id="%s" value="%s" %s /> <span>%s</span></p>', OPICNM_Input_SLUG.$get_attr_name, $get_attr_id, $key, $checked, $value);
		}
